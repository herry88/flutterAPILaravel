...
class _PageMainSiswaState extends State<PageMainSiswa> {
  Future<List> getData() async {
    final response = await http.get("http://172.10.2.9/ApiLaraFlutter/public/api/siswa");
      ...
      itemBuilder: (context, i){
        return new Container(
          child: new GestureDetector(
            ...
            child: new Card(
              color: Colors.white,
              child: ListTile(title: Text(widget.list[i]['nama']),
                leading: Icon(Icons.list),
                subtitle: Text('Alamat : ${widget.list[i]['alamat']}'),
              ),
            ),
          ),
        );
...
//  void deleteData(){
//    var url = "";
//    http.post(url,body: {'id' : widget.list[widget.]})
//  }
//  @override
//  Widget build(BuildContext context) {
//
